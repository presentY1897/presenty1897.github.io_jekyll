...
_대중교통과 자가용의 최단 경로 찾기_ 을 주제로 데이터 취득 및 구축에서 웹서비스 수준까지 간단한 샘플을 만들어봅니다.
서울시를 배경으로 대중교통이나 자가용을 이용한 경로탐색 지도를 만드는 과정을 설명합니다.
최대한 깔끔[^1]하게 설명하고자 노력했습니다.

포스팅 내용은 크게 두 부분으로 나뉩니다. **도로망을 이용한 자가용 경로 탐색**과 **대중교통 API를 이용한 대중교통 경로 탐색**입니다.
진행 과정은 데이터 취득, 데이터 처리 및 구축, 경로 탐색 알고리즘 구현 순으로 자가용과 대중교통 두 부분 모두 동일합니다.
...
대중교통 API를 이용한 대중교통 경로 탐색은 API를 이용하여 대중교통 망과 운행시간표를 취득하고, [RAPTOR(Round-Based Public Transit Routing)](https://www.microsoft.com/en-us/research/wp-content/uploads/2012/01/raptor_alenex.pdf)로 경로를 탐색합니다.
대중교통 경로탐색 알고리즘인 RAPTOR는 해당 문서를 참고하여 구현하였습니다.

툴과 데이터는 모두 오픈 소스를 사용합니다.

## 환경

### 개발 도구

다음과 같은 개발 도구가 사용됩니다. 이 포스트에서는 윈도우 환경(Window 10)을 기준으로 작성합니다. 도구 뒤에 작성된 버전은 제가 사용한 버전입니다.
...
* [QGIS >= 3.6.1](https://qgis.org/ko/site/forusers/download.html): 데이터 확인(좌표 변환)
* [GeoSERVER >= 2.15.0](http://geoserver.org/): 데이터베이스의 데이터를 지도에 가시화하기 위한 서버(WMS)

### 데이터

이 포스팅은 다음의 데이터를 활용합니다. 데이터는 문서 작성 시점(2019/04/12)을 기준으로 가져왔습니다.
